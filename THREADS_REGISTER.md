...
| PRE_REG_015 | Climate-attribution USA 3a-3b | **locked + ALL TESTABLE SETS FIRED (11/11)** | 2026-05-27 | Set A CONSISTENT (7.1%→17.6%); **Set C SUPPORTED: r(IDP, ACE)=+0.552**; **Set E SUPPORTED: CUB replicates, F5 NOT FIRED**; **Set B CONSISTENT-BUT-FRAGILE: HadISST extended to 2025; r(IDP, MDR-SST)=+0.518 raw GIDD-window but outlier-driven (log +0.23 / Spearman +0.15); 1980-2003 +0.24; pooled +0.21. Direct SST link weak; robust mechanism is SST→ACE→displacement**; Set D forward-watch. F1/F2/F3/F5 all NOT FIRED. See digs hadisst_hurdat_ace + setE_cub_pri + setB_sst |
...
- **C-Ours-60 — PRE_REG_022 robustness: typology IRREDUCIBLE to off-the-shelf country covariates** — Two follow-up tests fired 2026-05-27: (1) Ablation: disaster regimes alone +11.28; conflict types alone −1.53 (structurally disadvantaged); combined +14.22 — super-additive (interaction = ~+4.5 above sum of parts). (2) Latent tests: WDI demographic-economic (log_pop + log_gdp_pc + urban_share) absorbs +1.28 alone, typology survives +12.48 incremental. Polity (polity2 + durable) absorbs −0.42 alone, typology survives +12.92 incremental. **Neither demographic-exposure NOR state-fragility/democratic-institutions explains the typology lift.** Typology encodes multi-dimensional joint structure (hazard × conflict-form) that no single canonical country scalar approximates. Paper 6 methodology claim strengthened.
...
- **C-Ours-61 — PRE_REG_015 Set E SUPPORTED (CUB replicates USA storm intensification)** — CUB mega-year frequency 7.1% (1980-2007) → 17.6% (2008-2024), identical to USA trajectory; per-capita mega-years 4→6 (+21%). PRI shows no intensification but is a structurally weak test member: max storm-IDP only 88K because Hurricane Maria 2017 displacement was dominated by off-island emigration to US mainland (out-migration, not IDP). **F5 does NOT fire.** First-mover / Regime-3a-wide intensification hypothesis gains a second confirming Atlantic member (CUB). Paper 2 climate-attribution thread now has 3 confirming points (Set A freq + Set C ACE + Set E CUB). Caveat: cross-source metric (EM-DAT affected pre-2008 vs GIDD-IDP post-2008); bias uniform + conservative for trend test.
- **C-Ours-64 — PRE_REG_015 Set B CONSISTENT-BUT-FRAGILE; Paper 2 → 11/11** — HadISST extended to 1870-2025 (pulled per-year text files 2004-2025 from Met Office; 2024 Aug-Oct MDR SST = 29.05°C, warmest in series). r(USA storm-IDP, MDR SST): 1980-2003 +0.238; **2008-2024 GIDD-native +0.518 raw (crosses +0.4) BUT outlier-driven — log +0.232 / Spearman +0.150 collapse** (carried by 2017 + 2024 record years); pooled 1980-2024 +0.205. F2 NOT FIRED (+0.518 primary) but robustness near floor. **Mechanism refinement: direct SST→displacement link is weak/noisy because SST predicts basin ACE but US displacement needs landfall; robust chain is SST→ACE→displacement (Set C +0.552 is the load-bearing intermediate).** All testable PRE_REG_015 sets now fired → Paper 2 11/11. Climate-attribution thread anchored by Set C (ACE) + Set E (CUB); Set B is upstream-but-noisy.
...
| ID | Title | Status | Last update | Notes |
|---|---|---|---|---|
| PRE_REG_022 | Residue-class Stan model > classical baseline | **locked + fit fired + H1 SUPPORTED + robustness CONFIRMED** | 2026-05-27 | Headline ΔLOO = +13.66 (predicted ≥ 5); ablation: disaster regimes +11.28, conflict types +2.94 incremental, super-additive combined +14.22. Latent tests: WDI demographic-economic +1.28 (rejects), Polity +(−0.42) (rejects). **Typology irreducible to off-the-shelf country covariates.** F1 NOT FIRED. See PAPER_6_METHODOLOGY/digs/2026_05_27_phase1_residue_class_fit.md + 2026_05_27_ablation_and_latent_tests.md |
...
| P5-A2 | PRE_REG_030 Set C out-of-sample scoring | **closed-supported** | 2026-05-28 | Scored 12 current democracies at 2024. AT-RISK: GEO/PHL/IND/MEX/IDN; PROTECTED: ZAF/ITA/ARG/DEU/FRA/GRC/ESP. Face-valid vs scholarly backsliding-watch; model distinguishes Milei-disruptive-but-constrained from Morena-court-capturing. Forward-falsifiable 2025-2030. See digs/2026_05_28_prereg030_setC_oos.md |
...
**Paper 2 substrate status**: **11 of 11 closure criteria met 2026-05-27.** All PRE_REG_015 testable sets fired (A consistent / B fragile-consistent / C supported +0.552 / E supported CUB; D forward-watch). Typology expanded to 7 regimes (Regime 7 Wildfire CONFIRMED via PRE_REG_029; R2b walked back). Climate-attribution thread anchored by ACE (Set C) + CUB replication (Set E); direct SST link (Set B) is upstream-but-noisy. HadISST extended to 1870-2025.
